...
async function prorrateoConfigurado(): Promise<Prorrateo> {
  ...
  const { data } = await sb.from("pr_config").select("metodo_prorrateo").eq("id", 1).single();
...
export async function crearPrestamo(d: DatosNuevoPrestamo) {
  ...
  const { error: e2 } = await sb.from("pr_cuotas").insert(filas);
  if (e2) {
    await sb.from("pr_prestamos").delete().eq("id", prestamo.id);
    throw e2;
  }
...
export async function actualizarMoras(prestamoId: string): Promise<number> {
  const sb = createAdminClient();
  const { data: p } = await sb.from("pr_prestamos").select("*").eq("id", prestamoId).single();
  ...
  for (const c of pendientes) {
    ...
    if (r2(mora) !== r2(Number(c.mora_generada))) {
      await sb.from("pr_cuotas").update({ mora_generada: mora }).eq("id", c.id);
    }
    total += mora;
  }
  return r2(total);
...
async function crearDistribucion(
  ...
  const { error } = await sb.from("pr_distribuciones").insert({
    pago_id: pagoId,
    prestamo_id: p.id,
...
async function sincronizarEstado(prestamoId: string) {
  ...
  const { data: p } = await sb.from("pr_prestamos").select("estado").eq("id", prestamoId).single();
  if (!p || p.estado === "cancelado") return;

  if (pendientes === 0 && p.estado === "activo") {
    await sb.from("pr_prestamos").update({ estado: "pagado" }).eq("id", prestamoId);
  } else if (pendientes > 0 && p.estado === "pagado") {
    await sb.from("pr_prestamos").update({ estado: "activo" }).eq("id", prestamoId);
  }
}
...
export async function aplicarReenganche(d: DatosReenganche) {
  ...
  await sb.from("pr_reenganches").insert({
    prestamo_id: p.id,
    fecha: d.fecha,
...
export async function anularPago(pagoId: string) {
  ...
  if (pago.cuota_id) {
    const { data: c } = await sb.from("pr_cuotas").select("*").eq("id", pago.cuota_id).single();
    if (c) {
  ...
  await sb.from("pr_distribuciones").delete().eq("pago_id", pagoId);
  await sb.from("pr_pagos").update({ anulado: true }).eq("id", pagoId);
