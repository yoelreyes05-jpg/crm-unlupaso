...
/**
 * Helper CRUD para las tablas rep_* del ERP Repostería.
 * Todas las rutas usan la service role key, así que RLS no aplica.
 */

export interface CrudOptions {
  /** Nombre exacto de la tabla, siempre con prefijo rep_ */
  table: string;
  ...
  orderBy?: string;
  ascending?: boolean;
  /** select() personalizado (para traer relaciones) */
  select?: string;
}
...
function assertTabla(table: string) {
  if (!table.startsWith(PREFIJO)) {
    throw new Error(`Tabla no permitida: ${table}. El ERP repostería solo usa tablas ${PREFIJO}*`);
  }
}
...
/** Atajo: devuelve los handlers GET/POST de una colección. */
...
/** Atajo: handlers GET/PATCH/DELETE de un recurso individual. */
