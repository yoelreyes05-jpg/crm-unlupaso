...
/**
 * Helper CRUD para las tablas ti_* del módulo de Tienda MAXMATT SHOP.
 * Mismo patrón que el módulo de préstamos: las rutas usan la service role key,
 * así que RLS no aplica y ningún módulo puede tocar las tablas del otro.
 */

export interface CrudOptions {
  /** Nombre exacto de la tabla, siempre con prefijo ti_ */
  table: string;
...
function assertTabla(table: string) {
  if (!table.startsWith(PREFIJO)) {
    throw new Error(`Tabla no permitida: ${table}. El módulo de tienda solo usa tablas ${PREFIJO}*`);
  }
}
