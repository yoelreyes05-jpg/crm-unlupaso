...
import { actualizar, eliminar, fail } from "@/lib/prestamos/crud";
import { PRESTAMOS } from "@/lib/prestamos/tablas";
import { actualizarMoras } from "@/lib/prestamos/motor";
...
export async function GET(_req: NextRequest, ctx: Ctx) {
  try {
    ...
    const [prestamo, cuotas, pagos, reenganches] = await Promise.all([
      sb.from("pr_v_prestamos").select("*").eq("id", id).single(),
      sb.from("pr_v_cuotas").select("*").eq("prestamo_id", id).order("ciclo").order("numero"),
      sb.from("pr_pagos").select("*").eq("prestamo_id", id).order("fecha", { ascending: false }),
      sb.from("pr_reenganches").select("*").eq("prestamo_id", id).order("fecha", { ascending: false }),
    ]);
...
export async function DELETE(_req: NextRequest, ctx: Ctx) {
  const { id } = await ctx.params;
  const sb = createAdminClient();
  const { data } = await sb.from("pr_pagos").select("id").eq("prestamo_id", id).limit(1);
  if (data && data.length > 0) {
    // Con pagos registrados no se borra: se cancela para no perder la contabilidad
    const { error } = await sb.from("pr_prestamos").update({ estado: "cancelado" }).eq("id", id);
