...
import { VISTAS_PERMITIDAS } from "@/lib/prestamos/tablas";

/** Lectura genérica de las vistas pr_v_* (solo las de la lista blanca). */
export async function GET(req: NextRequest, ctx: { params: Promise<{ nombre: string }> }) {
  try {
    const { nombre } = await ctx.params;
    const vista = nombre.startsWith("pr_v_") ? nombre : `pr_v_${nombre}`;
