...
export async function GET(req: Request) {
  try {
    ...
    const [kpis, meses, porInv, dist, cartera, inversionistas] = await Promise.all([
      ...
      sb.from("pr_v_ganancias_mensuales").select("*").limit(24),
      sb.from("pr_v_ganancias_inversionista").select("*").limit(300),
      sb.from("pr_distribuciones").select("*").gte("fecha", desde).lte("fecha", hasta),
      sb.from("pr_v_prestamos").select("*").order("created_at", { ascending: false }).limit(500),
      sb.from("pr_inversionistas").select("id,nombre,codigo"),
    ]);
