...
import { VISTAS_PERMITIDAS } from "@/lib/anyeli/tablas";

/** Lectura genérica de las vistas ia_v_* (solo las de la lista blanca). */
export async function GET(req: NextRequest, ctx: { params: Promise<{ nombre: string }> }) {
  try {
    const { nombre } = await ctx.params;
    const vista = nombre.startsWith("ia_v_") ? nombre : `ia_v_${nombre}`;
