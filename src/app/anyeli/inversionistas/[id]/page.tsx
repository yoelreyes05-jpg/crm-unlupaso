...
import type { PrInversionistaVista, PrMovimientoInversionista, PrPrestamoVista } from "@/types/anyeli";
...
export default function DetalleInversionista() {
    ...
    (async () => {
      try {
        const [vi, pr, mv, gi, cfg] = await Promise.all([
          api<{ data: PrInversionistaVista[] }>(`/vistas/ia_v_inversionistas?id=${id}`),
          api<{ data: PrPrestamoVista[] }>(`/creditos?inversionista_id=${id}`),
          api<{ data: PrMovimientoInversionista[] }>(`/movimientos?inversionista_id=${id}`),
          api<{ data: GananciaInv[] }>(`/vistas/ia_v_ganancias_inversionista?inversionista_id=${id}`),
  ...
  return (
    <div>
      <Link href="/anyeli/inversionistas" style={{ fontSize: 12, color: T.acento, textDecoration: "none" }}>
        ← Inversionistas
      </Link>
      <div style={{ height: 8 }} />
      <Titulo
          ...
          <Tabla
            ...
            columnas={[
              { name: "codigo", label: "Código",
                fmt: (v, f) => <Link href={`/anyeli/creditos/${f.id}`} style={{ color: "inherit", fontWeight: 700 }}>{String(v)}</Link> },
