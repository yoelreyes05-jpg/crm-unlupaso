...
import type { PrDashboard, PrGananciaMensual, PrPrestamoVista } from "@/types/prestamos";
              ...
              columnas={[
                { name: "nombre", label: "Inversionista",
                  fmt: (v, f) => f.id === "propio"
                    ...
                    : <Link href={`/prestamos/inversionistas/${f.id}`} style={{ color: T.texto, fontWeight: 700, textDecoration: "none" }}>{String(v)}</Link> },
        ...
        <Tabla
          ...
          columnas={[
            { name: "codigo", label: "Préstamo",
              fmt: (v, f) => <Link href={`/prestamos/creditos/${f.id}`} style={{ color: "inherit", fontWeight: 700, textDecoration: "none" }}>{String(v)}</Link> },
