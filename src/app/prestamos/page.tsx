...
import type { PrCuotaVista, PrDashboard, PrGananciaMensual } from "@/types/prestamos";
      ...
      <Aviso texto={
        <>
          ...
          <div style={{ marginTop: 6 }}>
            Ejecuta <code>supabase/prestamos_schema.sql</code> en Supabase y revisa{" "}
            <a href="/api/prestamos/salud" style={{ color: "inherit" }}>/api/prestamos/salud</a>.
          </div>
        </>
      } />
    );
  ...
  return (
    <div>
      <Titulo
        ...
        acciones={<Link href="/prestamos/creditos/nuevo" style={{ textDecoration: "none" }}><Btn>+ Nuevo préstamo</Btn></Link>}
      />
      ...
      <div style={{ display: "grid", gap: 18, gridTemplateColumns: "repeat(auto-fit, minmax(430px, 1fr))", marginBottom: 18 }}>
        <Seccion
          ...
          accion={<Link href="/prestamos/cobranza" style={{ fontSize: 12, fontWeight: 700, color: T.acento, textDecoration: "none" }}>Ver cobranza →</Link>}
          style={{ padding: 0 }}
        >
            ...
            <Tabla
              ...
              columnas={[
                { name: "cliente_nombre", label: "Cliente",
                  fmt: (v, f) => (
                    <Link href={`/prestamos/creditos/${f.prestamo_id}`} style={{ color: "inherit", fontWeight: 700 }}>
                      {String(v)}
          ...
          {d.proximas.length === 0 ? (
            ...
            <Tabla
              columnas={[
                { name: "cliente_nombre", label: "Cliente",
                  fmt: (v, f) => (
                    <Link href={`/prestamos/creditos/${f.prestamo_id}`} style={{ color: T.texto, fontWeight: 600, textDecoration: "none" }}>
                      {String(v)}
