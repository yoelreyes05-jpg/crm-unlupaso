...
import type { PrPrestamoVista } from "@/types/prestamos";
  ...
  return (
    <div>
      <Titulo
        ...
        acciones={
          <Link href="/prestamos/creditos/nuevo" style={{ textDecoration: "none" }}>
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        {cargando ? <Cargando /> : filtrados.length === 0 ? (
          <Vacio
            ...
            accion={<Link href="/prestamos/creditos/nuevo" style={{ textDecoration: "none" }}><Btn>Crear el primero</Btn></Link>}
          />
        ) : (
          <Tabla
            ...
            columnas={[
              { name: "codigo", label: "Código",
                fmt: (v, f) => (
                  <Link href={`/prestamos/creditos/${f.id}`} style={{ color: "inherit", fontWeight: 700, textDecoration: "none" }}>
                    {String(v)}
                    {Number(f.ciclo) > 1 && (
              ...
              { name: "cliente_nombre", label: "Cliente",
                fmt: (v, f) => (
                  <Link href={`/prestamos/creditos/${f.id}`} style={{ color: "inherit", fontWeight: 600, textDecoration: "none" }}>
                    {String(v)}
              ...
              { name: "inversionista_nombre", label: "Inversionista",
                fmt: (v, f) => v
                  ? <Link href={`/prestamos/inversionistas/${f.inversionista_id}`} style={{ color: "inherit", textDecoration: "none" }}>{String(v)}</Link>
