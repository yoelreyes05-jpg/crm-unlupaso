...
import type { PrCliente, PrConfig, PrInversionistaVista } from "@/types/prestamos";
...
function NuevoPrestamo() {
    ...
    (async () => {
      try {
        const [cs, is, cfg] = await Promise.all([
          ...
          api<{ data: PrInversionistaVista[] }>("/vistas/pr_v_inversionistas?activo=true&orden=nombre"),
  ...
  async function crear() {
    ...
    try {
      ...
      router.push(`/prestamos/creditos/${r.data.id}`);
    } catch (e) {
            ...
            <div style={{ display: "grid", gap: 14, gridTemplateColumns: "1fr 1fr" }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: T.suave, marginTop: 4 }}>
                  ¿No está? <Link href="/prestamos/clientes" style={{ color: T.acento }}>Regístralo aquí</Link>
                </div>
              </div>
              <div>
