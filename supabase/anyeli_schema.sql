-- =====================================================================
--  MÓDULO  ·  INVERSIONES ANYELI
--  Prefijo de tablas: ia_        ·        Ruta de la app: /anyeli
-- =====================================================================
--  Copia independiente del sistema de préstamos. NO comparte ninguna
--  tabla con los demás módulos del proyecto (ul_, rep_, pr_, ti_):
--  clientes, inversionistas, préstamos, cuotas, pagos y contabilidad
--  son exclusivos de INVERSIONES ANYELI.
--
--  Ejecutar completo en Supabase → SQL Editor. Es idempotente:
--  se puede volver a correr cuantas veces haga falta.
--
--  Contenido:
--    PARTE 1 · 9 tablas + 7 vistas del módulo
--    PARTE 2 · nombre histórico del inversionista y borrado permitido
-- =====================================================================


-- #####################################################################
-- PARTE 1 · TABLAS Y VISTAS
-- #####################################################################

-- =====================================================================
--  ACTUALIZAR EL MÓDULO DE PRÉSTAMOS  (prefijo ia_)
-- =====================================================================
--  Deja la base exactamente como la necesita /anyeli, sin importar
--  desde dónde partas:
--
...
-- =====================================================================
drop view if exists ia_v_cobranza               cascade;
drop view if exists ia_v_dashboard              cascade;
drop view if exists ia_v_ganancias_inversionista cascade;
drop view if exists ia_v_ganancias_mensuales    cascade;
drop view if exists ia_v_inversionistas         cascade;
drop view if exists ia_v_prestamos              cascade;
drop view if exists ia_v_cuotas                 cascade;
...
-- =====================================================================
drop trigger  if exists ia_tg_perfil_nuevo on auth.users;
drop function if exists ia_fn_perfil_nuevo()          cascade;
drop function if exists ia_fn_rol()                   cascade;
drop function if exists ia_fn_es_staff()              cascade;
drop function if exists ia_fn_inversionista_actual()  cascade;
drop function if exists ia_fn_anular_pago(uuid)       cascade;  -- ahora se anula desde /api/anyeli/anular-pago
drop table    if exists ia_perfiles                   cascade;
...
begin
  for r in
    ...
    where schemaname = 'public' and tablename like 'ia\_%'
  loop
...
alter table if exists ia_pagos     drop column if exists registrado_por;
alter table if exists ia_prestamos drop column if exists prestamo_origen_id;
...
-- ia_config: tasa_admin_default → tasa_cliente_default
do $$
begin
  if exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='ia_config' and column_name='tasa_admin_default')
     and not exists (select 1 from information_schema.columns
             where table_schema='public' and table_name='ia_config' and column_name='tasa_cliente_default')
  then
    alter table ia_config rename column tasa_admin_default to tasa_cliente_default;
    raise notice 'Renombrada ia_config.tasa_admin_default → tasa_cliente_default';
  end if;
end $$;
...
-- =====================================================================
create sequence if not exists ia_seq_cliente       start 1;
create sequence if not exists ia_seq_inversionista start 1;
create sequence if not exists ia_seq_prestamo      start 1;
create sequence if not exists ia_seq_recibo        start 1;

create table if not exists ia_config (
  id                   int primary key default 1 check (id = 1),
  nombre_empresa       text not null default 'INVERSIONES ANYELI',
  rnc                  text,
  telefono             text,
  direccion            text,
...
insert into ia_config (id) values (1) on conflict (id) do nothing;

create table if not exists ia_clientes (
  id              uuid primary key default gen_random_uuid(),
  codigo          text unique not null default ('CLI-' || lpad(nextval('ia_seq_cliente')::text, 4, '0')),
...
create table if not exists ia_inversionistas (
  id                   uuid primary key default gen_random_uuid(),
  codigo               text unique not null default ('INV-' || lpad(nextval('ia_seq_inversionista')::text, 3, '0')),
  nombre               text not null,
  cedula               text,
  telefono             text,
  email                text,
...
create table if not exists ia_movimientos_inversionista (
  id               uuid primary key default gen_random_uuid(),
  inversionista_id uuid not null references ia_inversionistas(id) on delete cascade,
...
create table if not exists ia_prestamos (
  id                 uuid primary key default gen_random_uuid(),
  codigo             text unique not null default ('PR-' || lpad(nextval('ia_seq_prestamo')::text, 5, '0')),
  cliente_id         uuid not null references ia_clientes(id) on delete restrict,
  inversionista_id   uuid references ia_inversionistas(id) on delete restrict,
...
create table if not exists ia_cuotas (
  id                uuid primary key default gen_random_uuid(),
  prestamo_id       uuid not null references ia_prestamos(id) on delete cascade,
  ciclo             int not null default 1,
  numero            int not null,
...
create table if not exists ia_pagos (
  id            uuid primary key default gen_random_uuid(),
  recibo        text unique not null default ('REC-' || lpad(nextval('ia_seq_recibo')::text, 6, '0')),
  prestamo_id   uuid not null references ia_prestamos(id) on delete cascade,
  cuota_id      uuid references ia_cuotas(id) on delete set null,
  fecha         date not null default current_date,
...
create table if not exists ia_distribuciones (
  id                    uuid primary key default gen_random_uuid(),
  pago_id               uuid not null references ia_pagos(id) on delete cascade,
  prestamo_id           uuid not null references ia_prestamos(id) on delete cascade,
  inversionista_id      uuid references ia_inversionistas(id) on delete set null,
  fecha                 date not null default current_date,
...
create table if not exists ia_reenganches (
  id                   uuid primary key default gen_random_uuid(),
  prestamo_id          uuid not null references ia_prestamos(id) on delete cascade,
  fecha                date not null default current_date,
...
-- =====================================================================
alter table ia_config      add column if not exists logo_url             text;
alter table ia_config      add column if not exists tasa_cliente_default numeric(6,3) not null default 20;
alter table ia_config      add column if not exists created_at           timestamptz not null default now();
alter table ia_config      add column if not exists updated_at           timestamptz not null default now();

alter table ia_clientes    add column if not exists usuario    text;
alter table ia_clientes    add column if not exists created_at timestamptz not null default now();
alter table ia_clientes    add column if not exists updated_at timestamptz not null default now();

alter table ia_inversionistas add column if not exists usuario    text;
alter table ia_inversionistas add column if not exists created_at timestamptz not null default now();
alter table ia_inversionistas add column if not exists updated_at timestamptz not null default now();

alter table ia_movimientos_inversionista add column if not exists usuario    text;
alter table ia_movimientos_inversionista add column if not exists created_at timestamptz not null default now();

alter table ia_prestamos   add column if not exists usuario    text;
alter table ia_prestamos   add column if not exists created_at timestamptz not null default now();
alter table ia_prestamos   add column if not exists updated_at timestamptz not null default now();

alter table ia_cuotas      add column if not exists created_at timestamptz not null default now();

alter table ia_pagos       add column if not exists usuario    text;
alter table ia_pagos       add column if not exists created_at timestamptz not null default now();

alter table ia_distribuciones add column if not exists created_at timestamptz not null default now();

alter table ia_reenganches add column if not exists interes_capitalizado numeric(14,2) not null default 0;
alter table ia_reenganches add column if not exists usuario              text;
alter table ia_reenganches add column if not exists created_at           timestamptz not null default now();

-- =====================================================================
-- PASO 6 · Normalizar restricciones e índices
-- =====================================================================
-- El estado 'reenganchado' ya no existe: el reenganche deja el préstamo activo
update ia_prestamos set estado = 'activo' where estado = 'reenganchado';
alter table ia_prestamos drop constraint if exists ia_prestamos_estado_check;
alter table ia_prestamos add  constraint ia_prestamos_estado_check
  check (estado in ('activo','pagado','cancelado'));

create index        if not exists ia_idx_clientes_nombre on ia_clientes (lower(nombre));
create index        if not exists ia_idx_clientes_cedula on ia_clientes (cedula);
create index        if not exists ia_idx_mov_inv         on ia_movimientos_inversionista (inversionista_id, fecha);
create index        if not exists ia_idx_prestamos_cliente on ia_prestamos (cliente_id);
create index        if not exists ia_idx_prestamos_inv     on ia_prestamos (inversionista_id);
create index        if not exists ia_idx_prestamos_estado  on ia_prestamos (estado);
create unique index if not exists ia_idx_cuota_unica       on ia_cuotas (prestamo_id, ciclo, numero);
create index        if not exists ia_idx_cuotas_venc       on ia_cuotas (fecha_vencimiento) where activa;
create index        if not exists ia_idx_pagos_prestamo    on ia_pagos (prestamo_id, fecha);
create index        if not exists ia_idx_pagos_fecha       on ia_pagos (fecha) where not anulado;
create index        if not exists ia_idx_dist_inv          on ia_distribuciones (inversionista_id, fecha);
create index        if not exists ia_idx_dist_fecha        on ia_distribuciones (fecha);
create index        if not exists ia_idx_reenganches       on ia_reenganches (prestamo_id, fecha);

-- =====================================================================
-- PASO 7 · Vistas (versión corregida: 'solo_interes' cuenta como saldada)
-- =====================================================================
create view ia_v_cuotas as
select
  c.*,
...
from ia_cuotas c
join ia_prestamos p on p.id = c.prestamo_id
join ia_clientes  cl on cl.id = p.cliente_id;

create view ia_v_prestamos as
with agg as (
  select
    prestamo_id,
...
from ia_prestamos p
join ia_clientes cl on cl.id = p.cliente_id
left join ia_inversionistas i on i.id = p.inversionista_id
left join agg a on a.prestamo_id = p.id;

create view ia_v_inversionistas as
with mov as (
  select
    inversionista_id,
  ...
  from ia_movimientos_inversionista
...
create view ia_v_ganancias_mensuales as
select
...
from ia_distribuciones d
group by 1
order by 1 desc;

create view ia_v_ganancias_inversionista as
select
  d.inversionista_id,
  ...
  sum(d.interes_administrador + d.mora_administrador) as ganancia_administrador
from ia_distribuciones d
left join ia_inversionistas i on i.id = d.inversionista_id
group by 1,2,3
order by 3 desc;

create view ia_v_dashboard as
select
  (select count(*) from ia_clientes where activo)                                     as clientes_activos,
  (select count(*) from ia_inversionistas where activo)                               as inversionistas_activos,
  (select count(*) from ia_prestamos where estado = 'activo')                         as prestamos_activos,
  (select coalesce(sum(capital),0) from ia_prestamos where estado = 'activo')         as capital_colocado,
  (select coalesce(sum(saldo_capital),0) from ia_v_prestamos where estado = 'activo') as capital_por_recuperar,
  (select coalesce(sum(saldo_total),0)   from ia_v_prestamos where estado = 'activo') as cartera_por_cobrar,
  (select count(*) from ia_v_prestamos where estado_visual = 'atrasado')              as prestamos_atrasados,
  (select coalesce(sum(pendiente + mora_pendiente),0) from ia_v_cuotas
     where estado_visual = 'atrasada' and activa)                                     as monto_atrasado,
  (select coalesce(sum(interes_total),0) from ia_distribuciones)                      as interes_cobrado_total,
  (select coalesce(sum(interes_administrador + mora_administrador),0) from ia_distribuciones) as ganancia_administrador,
  (select coalesce(sum(interes_inversionista),0) from ia_distribuciones)              as ganancia_inversionistas,
  (select coalesce(sum(interes_administrador + mora_administrador),0) from ia_distribuciones
     where fecha >= date_trunc('month', current_date))                                as ganancia_admin_mes,
  (select coalesce(sum(monto),0) from ia_pagos
     where not anulado and fecha >= date_trunc('month', current_date))                as cobrado_mes;

create view ia_v_cobranza as
select *
from ia_v_cuotas
where activa
...
-- =====================================================================
create or replace function ia_fn_touch()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;

drop trigger if exists ia_tg_touch_clientes  on ia_clientes;
create trigger ia_tg_touch_clientes  before update on ia_clientes
  for each row execute function ia_fn_touch();

drop trigger if exists ia_tg_touch_inv       on ia_inversionistas;
create trigger ia_tg_touch_inv       before update on ia_inversionistas
  for each row execute function ia_fn_touch();

drop trigger if exists ia_tg_touch_prestamos on ia_prestamos;
create trigger ia_tg_touch_prestamos before update on ia_prestamos
  for each row execute function ia_fn_touch();

drop trigger if exists ia_tg_touch_config    on ia_config;
create trigger ia_tg_touch_config    before update on ia_config
  for each row execute function ia_fn_touch();

-- =====================================================================
-- PASO 9 · Seguridad
--   RLS activo SIN políticas: nadie lee estas tablas con la anon key.
--   La app entra por /api/anyeli/* con la service role key, que la ignora.
-- =====================================================================
alter table ia_config                    enable row level security;
alter table ia_clientes                  enable row level security;
alter table ia_inversionistas            enable row level security;
alter table ia_movimientos_inversionista enable row level security;
alter table ia_prestamos                 enable row level security;
alter table ia_cuotas                    enable row level security;
alter table ia_pagos                     enable row level security;
alter table ia_distribuciones            enable row level security;
alter table ia_reenganches               enable row level security;

alter view ia_v_cuotas                  set (security_invoker = on);
alter view ia_v_prestamos               set (security_invoker = on);
alter view ia_v_inversionistas          set (security_invoker = on);
alter view ia_v_ganancias_mensuales     set (security_invoker = on);
alter view ia_v_ganancias_inversionista set (security_invoker = on);
alter view ia_v_dashboard               set (security_invoker = on);
alter view ia_v_cobranza                set (security_invoker = on);

commit;


-- #####################################################################
-- PARTE 2 · NOMBRE HISTÓRICO DEL INVERSIONISTA + PERMITIR BORRARLO
-- #####################################################################

-- =====================================================================
-- INVERSIONES ANYELI · Poder eliminar inversionistas
-- =====================================================================
--
...
-- ---------------------------------------------------------------------
alter table ia_prestamos      add column if not exists inversionista_nombre_hist text;
alter table ia_distribuciones add column if not exists inversionista_nombre_hist text;

update ia_prestamos p
...
create or replace function ia_fn_nombre_inversionista()
returns trigger language plpgsql as $$
begin
  if new.inversionista_id is not null
    ...
    select nombre into new.inversionista_nombre_hist
      from ia_inversionistas where id = new.inversionista_id;
  end if;
  return new;
end $$;

drop trigger if exists ia_tg_nombre_inv_prestamo on ia_prestamos;
create trigger ia_tg_nombre_inv_prestamo
  before insert or update of inversionista_id on ia_prestamos
  for each row execute function ia_fn_nombre_inversionista();

drop trigger if exists ia_tg_nombre_inv_distrib on ia_distribuciones;
create trigger ia_tg_nombre_inv_distrib
  before insert or update of inversionista_id on ia_distribuciones
  for each row execute function ia_fn_nombre_inversionista();
...
begin
  for r in
    select con.conname
     ...
     where con.contype = 'f'
       and rel.relname = 'ia_prestamos'
       and att.attname = 'inversionista_id'
  loop
    execute format('alter table ia_prestamos drop constraint %I', r.conname);
  end loop;
end $$;

alter table ia_prestamos
  add constraint ia_prestamos_inversionista_id_fkey
  foreign key (inversionista_id) references ia_inversionistas(id) on delete set null;

-- ---------------------------------------------------------------------
-- 3. Las vistas usan el nombre guardado cuando el inversionista ya no está
-- ---------------------------------------------------------------------
-- ia_v_prestamos usa p.*, y como ia_prestamos ganó una columna hay que
...
select
  (select count(*) from information_schema.tables
    where table_schema='public' and table_name like 'ia\_%' and table_type='BASE TABLE') as tablas,
  (select count(*) from information_schema.views
    where table_schema='public' and table_name like 'ia\_v\_%')                          as vistas,
  (select nombre_empresa from ia_config where id = 1)                                    as empresa,
  case
    when (select count(*) from information_schema.tables
           where table_schema='public' and table_name like 'ia\_%' and table_type='BASE TABLE') = 9
     and (select count(*) from information_schema.views
           where table_schema='public' and table_name like 'ia\_v\_%') = 7
     and exists (select 1 from information_schema.columns
                  where table_schema='public' and table_name='ia_prestamos'
                    and column_name='inversionista_nombre_hist')
    then '✅ todo en orden — 9 tablas, 7 vistas, INVERSIONES ANYELI listo'
