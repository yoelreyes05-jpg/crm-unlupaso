...
select
  ...
  coalesce(pr.capital_colocado, 0)  as capital_colocado,
  coalesce(pr.prestamos_activos, 0) as prestamos_activos,
  coalesce(d.capital_devuelto, 0)   as capital_recuperado,
  coalesce(d.interes_ganado, 0)     as interes_ganado,
  round(coalesce(mv.aportes,0) - coalesce(mv.retiros,0)
        - coalesce(pr.capital_colocado,0) + coalesce(d.capital_devuelto,0), 2) as capital_disponible,
  round(coalesce(pr.saldo_en_calle, 0), 2) as capital_en_calle
from pr_inversionistas i
left join (
  select inversionista_id,
         sum(monto) filter (where tipo = 'aporte')                      as aportes,
         sum(monto) filter (where tipo in ('retiro','retiro_ganancia')) as retiros
  from pr_movimientos_inversionista group by inversionista_id
) mv on mv.inversionista_id = i.id
left join (
  select inversionista_id,
         sum(capital)                                        as capital_colocado,
         count(*) filter (where estado = 'activo')           as prestamos_activos,
         sum(saldo_capital) filter (where estado = 'activo') as saldo_en_calle
  from pr_v_prestamos where inversionista_id is not null group by inversionista_id
) pr on pr.inversionista_id = i.id
left join (
  select inversionista_id,
         sum(capital_devuelto)      as capital_devuelto,
         sum(interes_inversionista) as interes_ganado
  from pr_distribuciones where inversionista_id is not null group by inversionista_id
) d on d.inversionista_id = i.id;
